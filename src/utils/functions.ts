...
export const getHeader = (
  ...
  if (!safeApplication) throw new Error('Application is required');
  if (!safeK1) throw new Error('k1 is required');
...
export const buildUrl = (path: string, isDownload = false): string => {
  ...
  const baseUrl = isDownload 
    ? EtherdocSdkConfig.apiDownload?.trim() || ''
    : EtherdocSdkConfig.apiManagement?.trim() || '';
